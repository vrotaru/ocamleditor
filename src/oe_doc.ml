...
struct
  ...
  (** find_name *)
  let find_name ~project ~(symbol : Oe.symbol) =
    let value_path = (*Symbol.concat_value_path symbol*) String.concat "." symbol.Oe.sy_id in
  ...
  let find_module ~project ~symbol =
    let module_list = read ~project ~symbol in
    let module_name = (*Symbol.get_module_name symbol*) List.hd symbol.Oe.sy_id in
